...
/*
步骤1: PoolManager 在后台线程执行第一个lambda
       userExists(account) 返回 bool
       自动包装成 QVariant

步骤2: 工作完成后，PoolManager 调用第二个lambda
       传入 QVariant result 参数

步骤3: 第二个lambda内部
       从捕获列表获取 callback (原始的回调函数)
       从参数列表获取 result (QVariant类型的结果)
       
步骤4: 执行
       if (callback) - 检查回调函数是否有效
       callback(result.toBool()) - 调用原始回调，传入bool结果
       */
