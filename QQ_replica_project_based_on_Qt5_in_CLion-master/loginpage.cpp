...
bool LoginPage::checkLogin(DatabaseManager& db) {
    ...
    /*
        时间轴：
    T1: userExistsAsync(account, callback);  ← 注册回调函数
    T2: ui->clear();                        ← 主线程立即执行
    T3: return true;                        ← 主线程立即返回
    T4: [后台]数据库查询执行...               ← 主线程已经在做其他事了
    T5: [后台]查询完成                       ← 主线程仍在做其他事
    T6: callback(result)被调用               ← 现在才执行回调函数
    T7: if (!exists) { ... }               ← 回调函数内容执行*/
    //3.异步验证流程
